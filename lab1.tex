...
When we perform scientific experiments, we often have to take multiple measurements of a certain quantity.  In this case, we need to be able to calculate the error of the average of these measurements so that we can treat it as a single measurement with a higher level of accuracy.  To do this, we calculate the standard error of our measurement, which is equal to the standard deviation (found using Eq.(1)) divided by the square root of the number of trials performed, as shown in Eq.(2).  
\begin{equation}
...
The main objective of this lab is to learn about errors in measurement and how they affect our final calculated values.
